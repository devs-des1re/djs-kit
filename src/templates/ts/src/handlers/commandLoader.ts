...
import { REST, Routes, SlashCommandBuilder, Client, ChannelType } from 'discord.js';
import { config } from '../config.js';
import type { CommandDescriptor } from '../builders/index.js';
  ...
  if (slashCommands.size > 0 && config.token && config.clientId && config.guildId) {
    ...
    for (const { desc } of slashCommands.values()) {
      ...
      for (const param of desc.params) {
        ...
        else if (param.type === ParamType.TextChannel) builder.addChannelOption(opt => buildOpts(opt).addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement, ChannelType.GuildVoice));
