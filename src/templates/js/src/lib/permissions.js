export function checkPermissions(member, config) {
    if (!config)
        return { allowed: true };
    // 1. Blacklisted Users
    ...
    if (config.blacklistedRoles && config.blacklistedRoles.length > 0) {
        const hasBlacklistedRole = config.blacklistedRoles.some((r) => member.roles.cache.has(r) || member.roles.cache.some((mr) => mr.name === r));
    ...
    if (hasAllowedUsers || hasAllowedRoles) {
        ...
        const roleAllowed = config.allowedRoles?.some((r) => member.roles.cache.has(r) || member.roles.cache.some((mr) => mr.name === r)) ?? false;
