...
import { REST, Routes, SlashCommandBuilder, ChannelType, PermissionsBitField, ContextMenuCommandBuilder, ApplicationCommandType } from 'discord.js';
import { config } from '../config.js';
import { ParamType } from '../builders/types.js';
import { logger } from '../lib/logger.js';
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);
function addParamOption(builder, param) {
    const buildOpts = (opt) => {
        opt
            .setName(param.name)
...
function validateCommand(desc, filePath) {
    ...
    if (slashCommands.size > 0 && config.token && config.clientId && config.guildId) {
        ...
        try {
            ...
            if (config.commandRegistration === 'global') {
                ...
                await Promise.all(config.guildIds.map(guildId => rest.put(Routes.applicationGuildCommands(config.clientId, guildId), { body })));
            }
            else {
